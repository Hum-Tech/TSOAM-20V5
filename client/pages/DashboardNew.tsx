...
const logSystemActivity = async (action: string, details: string) => {
  try {
    ...
    if (((error as any)?.name || "Error") !== "AbortError") {
      console.warn("System activity logging unavailable:", (error as any)?.message || String(error));
    }
  }
};
...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (hasValidAuth) {
        try {
          // Fetch real-time data from API services with authentication
          const [fetchedTransactions, fetchedMembers, fetchedSummary] = await Promise.all([
            ApiFinancialService.getAllTransactions().catch((error) => {
              console.log('Financial API authentication required, using local data');
              return [];
            }),
            ApiMemberService.getAllMembers().catch((error) => {
              console.log('Members API authentication required, using local data');
              return [];
            }),
            ApiFinancialService.getFinancialSummary().catch((error) => {
              console.log('Financial summary API authentication required, using local data');
              return null;
            })
          ]);
          apiTransactions = fetchedTransactions;
          apiMembers = fetchedMembers;
          apiSummary = fetchedSummary;
        } catch (apiError) {
          console.log('Dashboard API calls failed, using local data:', apiError);
        }
      } else {
        console.log('No authentication - Dashboard using local data only');
      }

      // Fetch local module data as fallback/supplement
      ...
      // Merge API data with local data for enhanced dashboard
      let totalIncome = 0;
      let totalExpenses = 0;

      if (apiSummary && typeof apiSummary === 'object') {
        totalIncome = (apiSummary as any).totalIncome || 0;
        totalExpenses = (apiSummary as any).totalExpenses || 0;
      }

      if (apiMembers && apiMembers.length > 0) {
        (membershipData.stats as any).totalMembers = apiMembers.length;
        (membershipData.stats as any).activeMembers = apiMembers.filter((m: any) => m.is_active).length;
      }
  ...
  const manualRefresh = async () => {
    ...
    try {
      ...
      // Trigger refresh across the entire system
