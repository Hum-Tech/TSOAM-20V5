...
export default function Finance() {
  ...
  const addTransaction = async () => {
    ...
    try {
      ...
      if (
        (error as any)?.message || String(error).includes("Failed to fetch") ||
        (error as any)?.message || String(error).includes("NetworkError")
      ) {
        alert(
          "Unable to save transaction to server. Transaction saved locally only.\nPlease check your internet connection and try again later.",
        );
      } else {
        alert(`Failed to add transaction: ${(error as any)?.message || String(error)}`);
      }
    }
  };
  ...
  const addExpense = async () => {
    ...
    try {
      ...
      if (
        ...
        alert(`Failed to add expense: ${(error as any)?.message || String(error)}`);
      }
    }
  };
